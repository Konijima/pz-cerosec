...
do
	...
	bad(state, c, "set -e", "sh: set: -e: unknown option")
	bad(state, c, "set -x", "sh: set: -x: unknown option")
	bad(state, c, "set -u", "sh: set: -u: unknown option")
end
...
do
	...
	bad(state, c, "unset -f f; f", "f: command not found")
