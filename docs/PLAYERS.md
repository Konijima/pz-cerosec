...
| `find <path>... [-name <glob>] [-type f\|d]` | walk a tree depth-first, one path a line, the directory before what is in it; `-print` is implied as POSIX says and is accepted anyway. `-name` matches the **last component** with `*`, `?` and `[…]` (a leading `!` or `^` negates a set); `-type f` keeps files, `-type d` directories, and both together must both be true. A link is a leaf: find does not follow one. A directory it may not read is named and not entered, and the walk goes on **unsuccessfully** — so `find / \| wc -l` as an ordinary account prints the paths instead of counting them, exactly as `cat good bad \| wc -l` does |
...
| `cut -d <delim> -f <list>` | keep those fields, split on a one-character delimiter (a TAB by default). A list is `1`, `1,4`, `3-5`, `2-` (to the end) or `-3` (from the first). A line with no delimiter in it comes through **whole**, which is what POSIX says |
