...
| [computer-mod.md](computer-mod.md) | What the Workshop mod *Computer Mod* (id `ComputerModkum`) does to the eight vanilla desktop tiles this mod also uses, read out of its source: the two hooks, the modData keys, and the one line that darkens a lit screen on every right-click. Written in French, and the design note behind `CeroSecCompatComputerMod.lua`. Its chapters 4 to 6 are a dual-boot design that is **not** built. |
